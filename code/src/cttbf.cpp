...
{
CTTBF::CTTBF(const VRPInstance& vrp, const std::vector<double>& p)
{
	this->vrp = vrp;
	...
	for (auto j: Vo) f->AddConstraint(ESUM(i:in(j), t_hat[i][j] + ESUM(m:M_nz[i][j], (1+theta(i,j,m)) * tt[i][j][m]) + ESUM(m:M[i][j], eta(i,j,m) * xx[i][j][m] + s[j]*xx[i][j][m])).EQ(t[j])); // (6)
